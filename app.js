...
const BUDGETS=[
{id:'smart',target:35,icon:'🧺',title:'Smart · ~35€',desc:'base economica + dispensa',items:[['uova',10,'pz'],['petto di pollo',650,'g'],['tonno al naturale',360,'g'],['ceci cotti',600,'g'],['lenticchie cotte',600,'g'],['fagioli cannellini',400,'g'],['pasta integrale',700,'g'],['pasta',300,'g'],['riso',700,'g'],['cous cous',350,'g'],['passata di pomodoro',850,'g'],['patate',1100,'g'],['zucchine',600,'g'],['carote',450,'g'],['broccoli',400,'g'],['spinaci',300,'g'],['piselli',300,'g'],['pomodori',600,'g'],['cipolla',250,'g'],['cipolla rossa',100,'g'],['olive',60,'g'],['limone',2,'pz'],['yogurt greco',350,'g'],['ricotta',250,'g'],['parmigiano',50,'g'],['pane integrale',400,'g'],['olio evo',250,'ml']]},
{id:'balanced',target:55,icon:'🥕',title:'Balanced · ~55€',desc:'varietà, pesce, legumi',items:[['uova',8,'pz'],['petto di pollo',500,'g'],['salmone',350,'g'],['merluzzo',400,'g'],['tonno al naturale',240,'g'],['ceci cotti',500,'g'],['lenticchie cotte',450,'g'],['fagioli cannellini',350,'g'],['pasta integrale',500,'g'],['pasta',250,'g'],['riso',500,'g'],['orzo',300,'g'],['cous cous',300,'g'],['passata di pomodoro',700,'g'],['patate',900,'g'],['zucchine',600,'g'],['broccoli',400,'g'],['spinaci',300,'g'],['pomodori',600,'g'],['carote',450,'g'],['peperoni',350,'g'],['piselli',300,'g'],['cipolla',300,'g'],['limone',2,'pz'],['yogurt greco',350,'g'],['ricotta',250,'g'],['feta',180,'g'],['parmigiano',50,'g'],['pane integrale',400,'g'],['olive',80,'g'],['olio evo',250,'ml']]},
{id:'plus',target:75,icon:'🍷',title:'Plus · ~75€',desc:'più scelta e più fresco',items:[['uova',8,'pz'],['petto di pollo',550,'g'],['fesa di tacchino',400,'g'],['salmone',350,'g'],['merluzzo',400,'g'],['orata',350,'g'],['tonno al naturale',240,'g'],['ceci cotti',500,'g'],['lenticchie cotte',450,'g'],['fagioli cannellini',350,'g'],['pasta integrale',500,'g'],['pasta',300,'g'],['riso',500,'g'],['orzo',300,'g'],['cous cous',350,'g'],['passata di pomodoro',700,'g'],['patate',900,'g'],['zucchine',600,'g'],['broccoli',400,'g'],['spinaci',300,'g'],['pomodori',600,'g'],['carote',450,'g'],['peperoni',350,'g'],['melanzane',350,'g'],['lattuga',250,'g'],['yogurt greco',400,'g'],['ricotta',250,'g'],['feta',180,'g'],['parmigiano',50,'g'],['pane integrale',400,'g'],['noci',100,'g'],['cipolla',300,'g'],['limone',3,'pz'],['olive',80,'g'],['piselli',300,'g'],['olio evo',250,'ml']]}
];
...
function toBase(name,q,u){return window.GNAM_UNITS?.toBase(name,q,u) || null}
function fromBase(name,q,u){return window.GNAM_UNITS?.fromBase(name,q,u) ?? null}
function baseAmount(q,u,name=''){const x=toBase(name,q,u);return x?x.value:0}
function baseDimension(q,u,name=''){const x=toBase(name,q,u);return x?.dim||null}
function availableFor(name, pantry){const n=normalizeName(name);let total=0;let dim=null;for(const i of pantry.filter(i=>normalizeName(i.name)===n&&i.qty>0).sort((a,b)=>a.expiry.localeCompare(b.expiry))){const c=toBase(n,i.qty,i.unit);if(!c)continue;if(dim==null)dim=c.dim;if(c.dim===dim)total+=c.value;}return total}
function scaleQty(q){return q*(Number(state.people)||1)/2}
function recipeIngredients(r){const servings=Math.max(1,Number(r.servings)||2);return (r.ingredients||[]).map(([n,q,u])=>({name:normalizeName(n),label:n,qty:q*(Number(state.people)||1)/servings,unit:u}))}
function recipeCovered(r,pantry){return recipeIngredients(r).every(x=>{const c=toBase(x.name,x.qty,x.unit);if(!c)return false;return availableFor(x.name,pantry)>=c.value-0.01})}
function consumeInto(pantry,r){for(const x of recipeIngredients(r)){const xb=toBase(x.name,x.qty,x.unit);if(!xb)continue;let need=xb.value;for(const item of pantry.filter(i=>normalizeName(i.name)===x.name&&i.qty>0).sort((a,b)=>a.expiry.localeCompare(b.expiry))){const cb=toBase(x.name,item.qty,item.unit);if(!cb||cb.dim!==xb.dim)continue;const used=Math.min(cb.value,need),remain=cb.value-used,newQty=fromBase(x.name,remain,item.unit);if(newQty==null)continue;item.qty=Math.max(0,newQty);need-=used;if(need<=0)break}}}
...
function ingredientCost(item,used){const meta=canonMeta(item.name);if(item.price!=null&&item.price>0){const full=baseAmount(item.qty,item.unit,item.name)||1;return item.price*(used/full)}return meta.cost*((baseAmount(used,item.unit,item.name)||used)/100)}
...
async function applyPreset(id){const b=BUDGETS.find(x=>x.id===id);if(!b)return;state.pantry=[];state.budgetTarget=b.target;state.budgetPreset=b.id;addPantryItems(b.items);state.plan=null;state.score=null;state.suggestions=[];checkShopping(true);await buildPlan();toast(`${b.title} caricata: menu e check aggiornati.`);window.scrollTo({top:0,behavior:'smooth'})}
...
function macroForItems(items){let k=0,p=0,c=0,f=0,fib=0;for(const i of items){const n=nutrientFor(i.name);if(!n)continue;const grams=baseAmount(i.qty,i.unit,i.name);const factor=grams/100;k+=n[0]*factor;p+=n[1]*factor;c+=n[2]*factor;f+=n[3]*factor;fib+=n[4]*factor}return {k:Math.round(k),p:Math.round(p),c:Math.round(c),f:Math.round(f),fiber:Math.round(fib)}}
...
function estimateRecipeMacros(r){const servings=Math.max(1,Number(r.servings)||2);const people=Math.max(1,Number(state.people)||1);const parts=[];for(const [n,q,u] of r.ingredients||[]){parts.push({name:normalizeName(n),qty:q*people/servings,unit:u})}const m=macroForItems(parts);return m.k?m:{k:520,p:25,c:60,f:17,fiber:5}}
function recipeIsValid(r){
  if(!r||!r.name||!Array.isArray(r.ingredients)||r.ingredients.length<4)return false;
  if(!Array.isArray(r.steps)||r.steps.length<(r.remote?2:4))return false;
  if(r.ingredients.some(([n,q,u])=>!n||!(Number(q)>0)||!u||String(n).startsWith('__UNMAPPED__')))return false;
  if(r.remote){
    if(!r.verifiedOnline||!r.sourceUrl||!r.sourceInstructions||r.sourceInstructions.length<180)return false;
  } else if(r.editorialStatus!=='curated') return false;
...
function searchWeeklyPlan(curated, source){
  const BEAM=12, PAIRS_PER_STATE=20;
  ...
  for(let day=0;day<7;day++){
    ...
    for(const st of next){
      const fp=[...st.working].filter(x=>x.qty>0).map(x=>`${normalizeName(x.name)}:${Math.round(baseAmount(x.qty,x.unit,x.name))}`).sort().join('|');
...
const REMOTE_ALIASES={
 'chicken':'petto di pollo','chicken breast':'petto di pollo','turkey':'fesa di tacchino','salmon':'salmone','tuna':'tonno al naturale','cod':'merluzzo','sea bass':'orata','bream':'orata','eggs':'uova','egg':'uova','chickpeas':'ceci cotti','chickpea':'ceci cotti','lentils':'lenticchie cotte','beans':'fagioli cannellini','cannellini beans':'fagioli cannellini','peas':'piselli','rice':'riso','pasta':'pasta','whole wheat pasta':'pasta integrale','couscous':'cous cous','cous cous':'cous cous','pearl barley':'orzo','barley':'orzo','potatoes':'patate','potato':'patate','courgettes':'zucchine','zucchini':'zucchine','broccoli':'broccoli','spinach':'spinaci','carrots':'carote','tomatoes':'pomodori','tomato':'pomodori','bell peppers':'peperoni','peppers':'peperoni','aubergine':'melanzane','eggplant':'melanzane','lettuce':'lattuga','onion':'cipolla','red onion':'cipolla rossa','lemon':'limone','greek yogurt':'yogurt greco','yogurt':'yogurt greco','ricotta':'ricotta','feta':'feta','parmesan':'parmigiano','bread':'pane integrale','wholemeal bread':'pane integrale','olives':'olive','walnuts':'noci','olive oil':'olio evo','extra virgin olive oil':'olio evo','garlic':'aglio','ginger':'zenzero','flour':'farina','plain flour':'farina'};
const REMOTE_STAPLES=new Set(['salt','pepper','black pepper','water']);
function remoteCanonical(n){const k=String(n||'').toLowerCase().replace(/[^a-zà-ÿ0-9\s-]/gi,' ').replace(/[-]+/g,' ').replace(/\s+/g,' ').trim();return REMOTE_ALIASES[k]||null}
function remoteQty(n,measure){
 const cn=remoteCanonical(n);if(!cn)return null;
 const m=String(measure||'').toLowerCase().replace(/–/g,'-').replace(/\s+/g,' ').trim();
 if(!m||/to taste|as needed|q\.?b\.?|for serving|for garnish|divided/.test(m))return null;
 if(/\b(can|cans|tin|tins|packet|pack|bag|bags|jar|jars)\b/.test(m))return null;
 let q=null;const frac=m.match(/(\d+(?:[\.,]\d+)?|\d+\s+\/\s+\d+)/);
 if(frac){const raw=frac[1].replace(',','.');q=raw.includes('/')?(()=>{const z=raw.split(/\s+\/\s+/);return Number(z[0])/Number(z[1])})():Number(raw);}
 if(q==null){const words={one:1,two:2,three:3,four:4,five:5,six:6,eight:8,ten:10,a:1,an:1};const w=m.split(/\s+/)[0];if(words[w]!=null)q=words[w];}
 if(q==null)return null;
 const matchers=[
  ['kilograms','kg'],['kilogram','kg'],['kg','kg'],['pounds','lb'],['pound','lb'],['lb','lb'],['ounces','oz'],['ounce','oz'],['oz','oz'],
  ['milliliters','ml'],['milliliter','ml'],['millilitres','ml'],['millilitre','ml'],['ml','ml'],['liters','l'],['liter','l'],['litres','l'],['litre','l'],['l','l'],
  ['grams','g'],['gram','g'],['g','g'],['cups','cup'],['cup','cup'],['tablespoons','tbsp'],['tablespoon','tbsp'],['tbsp','tbsp'],['teaspoons','tsp'],['teaspoon','tsp'],['tsp','tsp'],
  ['cloves','clove'],['clove','clove'],['stalks','stalk'],['stalk','stalk'],['sprigs','sprig'],['sprig','sprig'],['slices','slice'],['slice','slice'],['fillets','fillet'],['fillet','fillet'],['bunches','bunch'],['bunch','bunch'],['handfuls','handful'],['handful','handful']
 ];
 let unit='pz';for(const [needle,u] of matchers){if(new RegExp('(?:^|\\s)'+needle+'(?:\\s|$)','i').test(m)){unit=u;break;}}
 // Bare "1 medium onion" / "2 small potatoes" means pieces; map to a standard culinary weight.
 const sizeFactor=/\\blarge\\b/i.test(m)?1.35:/\\bsmall\\b/i.test(m)?0.7:1;
 q*=sizeFactor;
 const conv=window.GNAM_UNITS?.toBase(cn,q,unit);if(!conv)return null;
 if(conv.dim==='g')return [Number(conv.value.toFixed(2)),'g'];
 if(conv.dim==='ml')return [Number(conv.value.toFixed(2)),'ml'];
 if(conv.dim==='count')return [q,'pz'];
 return null;
}
...
function normalizeRemoteRecipe(r){
 const steps=Array.isArray(r.sourceSteps)&&r.sourceSteps.length?r.sourceSteps:cleanRemoteSteps(r.sourceInstructions||'');
 const integrity=Number(r.rawIngredientCount||0)>=3 && Number(r.mappedIngredientCount||0)===Number(r.rawIngredientCount||0) && steps.length>=3 && steps.every(x=>x.length>=28);
 return {...r,ingredients:(r.ingredients||[]).map(([n,q,u])=>[normalizeName(n),q,u]),steps,sourceUrl:r.sourceUrl||'',diff:2,time:estimateRemoteTime(steps,r),remoteIntegrity:integrity};
}
function estimateRemoteTime(steps,r){
 const t=String(r.sourceInstructions||'').toLowerCase();
...
function remoteCovered(r,pantry){return r.remoteIntegrity && recipeIngredients(r).every(x=>availableFor(x.name,pantry)>=baseAmount(x.qty,x.unit,x.name)-0.01)}
function qaRecipe(r){
 if(!r||!r.name||!Array.isArray(r.ingredients)||r.ingredients.length<3)return false;
 if(r.remote){if(!r.sourceUrl||!r.verifiedOnline||!r.remoteIntegrity)return false;if(!r.sourceInstructions||r.sourceInstructions.length<180)return false;if(!Array.isArray(r.steps)||r.steps.length<3||r.steps.some(s=>String(s).length<28))return false;}
 if(r.ingredients.some(([n,q,u])=>!n||!(Number(q)>0)||!u))return false;
 return true;
}

function groceryEstimatedCost(pantry){if(state.budgetTarget>0&&state.budgetPreset)return Number(state.budgetTarget.toFixed(2));return Number(pantry.reduce((sum,i)=>{if(i.price>0)return sum+i.price;const meta=canonMeta(i.name);return sum+(meta.cost||0)*(baseAmount(i.qty,i.unit,i.name)/100)},0).toFixed(2))}
function consumedMenuCost(plan){return Number((plan?.meals||[]).reduce((a,m)=>a+(m.recipe.cost||0),0).toFixed(2))}

function microDaySignals(pair){
 const names=new Set(pair.flatMap(m=>(m.recipe.ingredients||[]).map(x=>normalizeName(x[0]))));
 return {
  'Vitamina C':['peperoni','broccoli','pomodori','limone','spinaci','kiwi','arance'].some(x=>names.has(x)),
  'Folati':['spinaci','broccoli','ceci cotti','lenticchie cotte','fagioli cannellini','asparagi'].some(x=>names.has(x)),
  'Calcio':['ricotta','yogurt greco','feta','parmigiano','mozzarella','latte'].some(x=>names.has(x)),
  'Ferro':['lenticchie cotte','ceci cotti','fagioli cannellini','spinaci','carne','uova','seitan'].some(x=>names.has(x)),
  'Omega-3':['salmone','sgombro','sardine','alici','noci'].some(x=>names.has(x)),
  'Potassio':['patate','spinaci','broccoli','ceci cotti','lenticchie cotte','banane','avocado'].some(x=>names.has(x))
 };
}
function nutritionDay(pair){
 const ms=pair.map(m=>recipeMacroSummary(m.recipe));
 const total=ms.reduce((a,x)=>({kcal:a.kcal+x.kcal,p:a.p+x.p,c:a.c+x.c,f:a.f+x.f,fiber:a.fiber+x.fiber}),{kcal:0,p:0,c:0,f:0,fiber:0});
 const kcal=total.kcal||1; const pct={p:Math.round(total.p*4/kcal*100),c:Math.round(total.c*4/kcal*100),f:Math.round(total.f*9/kcal*100)};
 const foods=[...pair.flatMap(m=>m.recipe.ingredients||[])]; const names=new Set(foods.map(x=>normalizeName(x[0])));
 const msig=microDaySignals(pair);
 const signals={vegetables:['broccoli','spinaci','zucchine','carote','peperoni','pomodori','melanzane','lattuga','cavolfiore'].filter(x=>names.has(x)).length,fruit:['mela','banana','arance','frutta','kiwi'].filter(x=>names.has(x)).length,legumes:[...names].some(n=>canonMeta(n).group==='legume'),omega3:msig['Omega-3'],calcium:msig['Calcio'],iron:msig['Ferro'],folate:msig['Folati'],vitC:msig['Vitamina C'],potassium:msig['Potassio']};
 const macroOK=(pct.c>=45&&pct.c<=60)&&(pct.f>=20&&pct.f<=35)&&(pct.p>=12&&pct.p<=30);
 const microCount=[signals.omega3,signals.calcium,signals.iron,signals.folate,signals.vitC,signals.potassium].filter(Boolean).length;
 const microOK=signals.vegetables>=2 && microCount>=4;
 const dayFoodOK=signals.fruit>=1 || signals.vegetables>=3;
 const ok=macroOK&&microOK&&dayFoodOK;
 let score=0;
 score+=pct.c>=45&&pct.c<=60?25:pct.c>=40&&pct.c<=65?14:4;
 score+=pct.f>=20&&pct.f<=35?20:pct.f>=17&&pct.f<=38?11:4;
 score+=pct.p>=12&&pct.p<=30?15:pct.p>=10&&pct.p<=33?8:3;
 score+=total.fiber>=25?14:total.fiber>=18?10:total.fiber>=10?6:2;
 score+=signals.vegetables>=3?12:signals.vegetables>=2?8:3;
 score+=signals.legumes?4:0; score+=signals.omega3?4:0; score+=signals.calcium?3:0; score+=signals.iron?3:0;
 score=Math.max(0,Math.min(100,Math.round(score)));
 return {total,pct,signals,score,ok};
}
function dayRepairOptions(dn,pair){
 const o=[];
 if(dn.pct.c<45)o.push({name:'riso integrale',qty:80,unit:'g',why:'porta i carboidrati complessi verso il range della giornata'});
 if(dn.pct.c>60)o.push({name:'verdure',qty:250,unit:'g',why:'abbassa la densità energetica e aumenta fibra/micronutrienti'});
 if(dn.pct.f<20)o.push({name:'olio evo',qty:10,unit:'g',why:'aggiunge grassi prevalentemente insaturi'});
 if(dn.pct.f>35)o.push({name:'verdure',qty:200,unit:'g',why:'aumenta il volume del pasto senza aggiungere molti grassi'});
 if(dn.pct.p<12)o.push({name:'yogurt greco',qty:170,unit:'g',why:'aggiunge proteine e calcio'});
 if(dn.total.fiber<18)o.push({name:'lenticchie cotte',qty:150,unit:'g',why:'aumenta fibra, folati e proteine vegetali'});
 if(dn.signals.vegetables<2)o.push({name:'broccoli',qty:200,unit:'g',why:'porta più vicino il target di verdure e vitamina C/folati'});
 if(!dn.signals.calcium)o.push({name:'yogurt greco',qty:170,unit:'g',why:'fonte pratica di calcio'});
 if(!dn.signals.omega3)o.push({name:'noci',qty:20,unit:'g',why:'fonte di ALA e grassi insaturi'});
 if(!dn.signals.vitC)o.push({name:'peperoni',qty:150,unit:'g',why:'rafforza la copertura di vitamina C'});
 if(!dn.signals.iron)o.push({name:'lenticchie cotte',qty:150,unit:'g',why:'rafforza ferro e folati'});
 return o.filter((x,i,a)=>a.findIndex(y=>y.name===x.name)===i).slice(0,3);
}
...
async function buildPlan(){
 ...
 const localPool=(typeof RECIPES!=='undefined'&&Array.isArray(RECIPES)?RECIPES:[]).filter(r=>recipeAllowed(r)&&qaRecipe(r));
 const curated=[...localPool];
 $('#recipePoolStatus').textContent=`🍽️ ${curated.length} ricette candidate · ${localPool.length} curate · planner offline, nessuna API remota`;
 let result=searchWeeklyPlan(curated,source);
 if(result.meals.length<14){
   const relaxed=searchWeeklyPlanRelaxed(curated,source);
   if(relaxed.meals.length>result.meals.length) result=relaxed;
 }
 ...
 const dayDetails=[];for(let d=0;d<7;d++){const pair=meals.filter(m=>m.day===d);if(pair.length===2){const dn=nutritionDay(pair);dayDetails[d]={...dn,repair:dn.ok?[]:dayRepairOptions(dn,pair)};}else dayDetails[d]=null;}
 state.plan={start:state.weekStart,meals,remaining:result.working||source,dayScores:dayDetails.map(x=>x?.score||0),dayDetails};
 state.plan.repairSuggestions=dayDetails.map((x,d)=>x&&!x.ok?{day:d,options:x.repair}:null).filter(Boolean);
 for(const m of meals)state.prefs.seen[m.recipe.name]=(state.prefs.seen[m.recipe.name]||0)+1;
 save();buildSuggestions();renderAll();
 const bad=dayDetails.filter(x=>x&&!x.ok).length, covered=meals.length;
 toast(covered<14?`Ho coperto ${Math.floor(covered/2)} giorni. ${bad?`${bad} giornate hanno anche una proposta di riequilibrio.`:''}`:bad?`14 pasti pronti: ${bad} giornate richiedono una piccola aggiunta per diventare nutrizionalmente OK.`:'14 pasti pronti: tutte le giornate passano il check nutrizionale.' );
}
function searchWeeklyPlanRelaxed(curated,source){
 const BEAM=18, MIN_BAL=68; let states=[{day:0,working:source.map(x=>({...x})),used:new Set(),meals:[],dayScores:[],score:0}];
 for(let day=0;day<7;day++){const next=[];for(const st of states){const pool=curated.filter(r=>recipeAllowed(r)&&!st.used.has(r.id)&&recipeCovered(r,st.working));const pairs=[];for(let i=0;i<pool.length;i++)for(let j=i+1;j<pool.length;j++){const a=pool[i],b=pool[j];if(a.protein&&b.protein&&a.protein===b.protein)continue;const overlap=pairOverlap(a,b);if(overlap.length>1)continue;const temp=st.working.map(x=>({...x}));consumeInto(temp,a);if(!recipeCovered(b,temp))continue;const bal=dayBalanceScore(a,b);if(bal<MIN_BAL)continue;const urgency=pairUrgency(a,b,st.working);const variety=hashString(a.id+'|'+b.id+'|'+day)%13;pairs.push({a,b,temp,bal,score:bal*2.6+urgency+variety});}pairs.sort((a,b)=>b.score-a.score);for(const p of pairs.slice(0,30)){const used=new Set(st.used);used.add(p.a.id);used.add(p.b.id);next.push({day:day+1,working:p.temp,used,meals:st.meals.concat([{day,slot:0,recipe:p.a},{day,slot:1,recipe:p.b}]),dayScores:st.dayScores.concat(p.bal),score:st.score+p.score});}}if(!next.length)break;next.sort((a,b)=>b.score-a.score);states=next.slice(0,BEAM);}
 const complete=states.find(s=>s.meals.length===14);return complete?{meals:complete.meals,working:complete.working,dayScores:complete.dayScores,score:complete.score}:{meals:[],working:source.map(x=>({...x})),dayScores:[],score:0};
}
function estimateRecipeCost(r,working){let total=0;const servings=Math.max(1,Number(r.servings)||2);for(const [n,q0,u] of r.ingredients){const q=q0*(Number(state.people)||1)/servings;let need=baseAmount(q,u,n);for(const item of working.filter(i=>normalizeName(i.name)===normalizeName(n)&&i.qty>0).sort((a,b)=>a.expiry.localeCompare(b.expiry))){const use=Math.min(baseAmount(item.qty,item.unit,n),need);total+=item.price>0?item.price*(use/baseAmount(item.qty,item.unit,n)):canonMeta(n).cost*(use/100);need-=use;if(need<=0)break}}return Number(total.toFixed(2))}
function renderPlan(){
 const g=$('#planGrid'),status=$('#planStatus');
 if(!state.plan||!state.plan.meals?.length){g.innerHTML='';status.textContent=state.pantry.length?'Premi “Genera 7 giorni” per costruire il calendario.':'Inserisci la spesa o scegli un preset.';$('#weeklySummary').classList.add('hidden');$('#dailyNutrition').classList.add('hidden');return}
 const byDay=DAY_NAMES.map((name,d)=>({name,d,meals:state.plan.meals.filter(x=>x.day===d)}));
 g.innerHTML=byDay.map(day=>{const date=addDays(state.plan.start,day.d);const dn=state.plan.dayDetails?.[day.d]||null;const repair=dn&&!dn.ok?dayRepairOptions(dn,day.meals):[];return `<article class="day-card"><div class="day-head"><div><b>${day.name}</b><small>${fmtDate(date)}</small></div><span class="day-score">${dn?`⚖ ${dn.score}/100`: `${day.meals.length}/2`}</span></div>${[0,1].map(slot=>{const m=day.meals.find(x=>x.slot===slot);return m?`<button class="meal-card ${slot?'dinner':''}" data-meal="${m.id}"><div class="meal-type">${slot?'CENA':'PRANZO'}</div><div class="meal-title">${escapeHtml(m.recipe.name)}</div><div class="meal-meta"><span>⏱ ${m.recipe.time||20}′</span><span>${m.recipe.kcal||520} kcal</span><span>${money(m.recipe.cost)}</span></div><div class="meal-bottom"><span>${m.recipe.sourceUrl?'✓ verificata':'curata'}</span><span class="cook-state ${state.cookLog[m.id]?'done':''}">${state.cookLog[m.id]?'✓ cucinato':'○ da fare'}</span></div></button>`:`<div class="meal-card empty-slot"><div class="meal-type">${slot?'CENA':'PRANZO'}</div><p>Non coperto dalla spesa</p></div>`}).join('')}<div class="day-nutrition"><b>${dn?(dn.ok?'🟢 Giornata OK':'🟠 Quasi OK'):'Dati insufficienti'}</b>${dn?`<span>${dn.total.kcal} kcal · P ${dn.total.p}g · C ${dn.total.c}g · G ${dn.total.f}g · fibra ${dn.total.fiber}g</span><small>Macro: C ${dn.pct.c}% · G ${dn.pct.f}% · P ${dn.pct.p}% · micronutrienti: ${dn.signals.vegetables>=2?'✓ verdure':'△ verdure'} ${dn.signals.legumes?'✓ legumi':'△ legumi'} ${dn.signals.omega3?'✓ omega-3':'△ omega-3'} ${dn.signals.calcium?'✓ calcio':'△ calcio'} · ${dn.signals.vitC?'✓ vit.C':'△ vit.C'} ${dn.signals.folate?'✓ folati':'△ folati'} ${dn.signals.iron?'✓ ferro':'△ ferro'} ${dn.signals.potassium?'✓ K':'△ K'}</small>${!dn.ok&&repair.length?`<div class="day-repair"><b>Per portarla a OK:</b>${repair.map(x=>`<span>+ ${escapeHtml(x.name)} ${x.qty}${x.unit} · ${escapeHtml(x.why)}</span>`).join('')}</div>`:''}`:''}</div></article>`}).join('');
 $$('[data-meal]').forEach(b=>b.onclick=()=>openMeal(b.dataset.meal));
 const totalK=state.plan.meals.reduce((a,m)=>a+(m.recipe.kcal||0),0),totalMenuCost=consumedMenuCost(state.plan), grocery=groceryEstimatedCost(state.pantry);
 const days=byDay.filter(x=>x.meals.length===2).map(x=>nutritionDay(x.meals)); const avgDayScore=days.length?Math.round(days.reduce((a,x)=>a+x.score,0)/days.length):0;
 const bad=days.filter(x=>!x.ok).length;
 status.innerHTML=`<b>${state.plan.meals.length}/14 pasti coperti</b> · ${Math.round(totalK/Math.max(1,state.plan.meals.length))} kcal medie/pasto · <b>${money(grocery)}</b> spesa stimata · ${money(totalMenuCost)} valore usato dal menu · ${bad?`<b>${bad} giornate da riequilibrare</b>`:'<b>7/7 giornate OK</b>'}`;
 $('#weeklySummary').innerHTML=`<div class="summary-stat"><b>${state.plan.meals.length}</b><span>pasti coperti</span></div><div class="summary-stat"><b>${money(grocery)}</b><span>costo della spesa</span></div><div class="summary-stat"><b>${money(totalMenuCost)}</b><span>valore ingredienti usati</span></div><div class="summary-stat"><b>${avgDayScore}/100</b><span>equilibrio medio delle giornate</span></div><div class="summary-note">Le ricette online entrano nel planner solo quando il testo originale, gli ingredienti e la fonte sono completi e coerenti. Se una giornata non può essere resa OK con ciò che hai, Gnam mostra ingredienti e porzioni concrete da aggiungere invece di fingere che sia bilanciata.</div>`;
 $('#weeklySummary').classList.remove('hidden');$('#dailyNutrition').classList.remove('hidden');
}
...
function consumeActual(r){for(const [n,q,u] of r.ingredients){const xb=toBase(n,q,u);if(!xb)continue;let need=xb.value;for(const item of state.pantry.filter(i=>normalizeName(i.name)===normalizeName(n)&&i.qty>0).sort((a,b)=>a.expiry.localeCompare(b.expiry))){const cb=toBase(n,item.qty,item.unit);if(!cb||cb.dim!==xb.dim)continue;const used=Math.min(cb.value,need),remain=cb.value-used,newQty=fromBase(n,remain,item.unit);if(newQty==null)continue;item.qty=Math.max(0,newQty);need-=used;if(need<=0)break}}}
...
function buildSuggestions(){
 ...
 for(const r of curated.filter(recipeIsValid)){
   ...
   const missing=[...new Set(recipeIngredients(r).filter(x=>availableFor(x.name,p)<baseAmount(x.qty,x.unit,x.name)).map(x=>normalizeName(x.name)))];
...
function webSearch(){const active=state.pantry.filter(i=>i.qty>0&&daysFromToday(i.expiry)>=0).sort((a,b)=>a.expiry.localeCompare(b.expiry));if(!active.length){toast('Prima inserisci qualcosa.');return}$('#webRecipesSection').classList.remove('hidden');const names=active.slice(0,4).map(x=>displayFood(x.name));const query=encodeURIComponent(names.join(' recipe '));const themeal=encodeURI('https://www.themealdb.com/search.php?s='+names[0]);const google='https://www.google.com/search?q='+query+'+recipe';$('#webGrid').innerHTML=`<article class="web-card"><div class="web-body"><h3>Ispirazione per ${escapeHtml(names.join(', '))}</h3><p>La ricerca viene aperta direttamente nel sito, senza chiamate API dal browser. Così Gnam non è bloccato da CORS o rate limit.</p><div class="web-links"><a class="primary-btn" href="${themeal}" target="_blank" rel="noreferrer">Cerca su TheMealDB ↗</a><a class="ghost-btn" href="${google}" target="_blank" rel="noreferrer">Cerca ricette sul web ↗</a></div></div></article>`}
...
function bind(){
 ...
 $('#ingredientForm').addEventListener('submit',e=>{e.preventDefault();const raw=$('#ingredientName').value.trim(),n=normalizeName(raw),qty=Number($('#ingredientQty').value),unit=$('#ingredientUnit').value,expiry=$('#ingredientExpiry').value,price=Number($('#ingredientPrice').value)||null;if(!raw||!qty||!expiry)return;const meta=canonMeta(n);state.pantry.push({id:`i-${Date.now()}-${Math.random()}`,name:n,original:raw,qty,unit,expiry,price,frozen:false});state.budgetTarget=null;state.budgetPreset=null;state.plan=null;state.score=null;state.suggestions=[];save();e.target.reset();$('#ingredientQty').value=meta.unit==='pz'?1:300;$('#ingredientUnit').value=meta.unit||'g';$('#ingredientExpiry').value=addDays(todayISO(),meta.days??7);$('#ingredientExpiry').dataset.manual='';renderAll();toast(`${raw} aggiunto.`)});
